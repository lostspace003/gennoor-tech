...
const AGENTS: Record<string, AgentConfig> = {
  profile_optimizer: {
    ...
    fields: [
      { key: 'headline', label: 'LinkedIn Headline', placeholder: 'e.g., Senior Software Engineer | Cloud Architecture | Azure Expert', type: 'text' },
      { key: 'about', label: 'About / Summary', placeholder: 'Paste your LinkedIn About section here...', type: 'textarea', rows: 4 },
      { key: 'experience', label: 'Experience', placeholder: 'Paste your work experience (roles, companies, key achievements)...', type: 'textarea', rows: 5 },
      { key: 'skills', label: 'Skills & Endorsements', placeholder: 'List your top skills (e.g., Python, Azure, Machine Learning)...', type: 'text' },
      { key: 'target_role', label: 'Target Role (Optional)', placeholder: 'What role are you targeting?', type: 'text' },
    ],
  },
  career_strategist: {
    ...
    fields: [
      { key: 'current_role', label: 'Current Role & Title', placeholder: 'e.g., Data Analyst at TCS, 3 years experience', type: 'text' },
      { key: 'skills', label: 'Current Skills', placeholder: 'List your technical and soft skills...', type: 'textarea', rows: 3 },
      { key: 'certifications', label: 'Certifications (Optional)', placeholder: 'e.g., AWS Solutions Architect, PMP...', type: 'text' },
      { key: 'target_goal', label: 'Career Goal / Target Role', placeholder: 'Where do you want to be?', type: 'textarea', rows: 2 },
  ...
  interview_coach: {
    ...
    fields: [
      { key: 'job_description', label: 'Job Description', placeholder: 'Paste the complete job description here...', type: 'textarea', rows: 5 },
      { key: 'resume', label: 'Your Resume / Profile', placeholder: 'Paste your resume or key experience...', type: 'textarea', rows: 4 },
      { key: 'company', label: 'Company Name (Optional)', placeholder: 'e.g., Microsoft, Google...', type: 'text' },
      { key: 'interview_type', label: 'Interview Type (Optional)', placeholder: 'e.g., Technical, Behavioral, System Design...', type: 'text' },
    ],
  },
  content_generator: {
    ...
    fields: [
      { key: 'expertise', label: 'Your Expertise Area', placeholder: 'e.g., AI Consulting, Cloud Architecture...', type: 'text' },
      { key: 'audience', label: 'Target Audience', placeholder: 'Who are you writing for?', type: 'textarea', rows: 2 },
      { key: 'tone', label: 'Preferred Tone', placeholder: 'e.g., Professional, Conversational, Thought-leadership...', type: 'text' },
      { key: 'topics', label: 'Key Topics / Themes', placeholder: 'What topics do you want to cover?', type: 'textarea', rows: 3 },
    ],
  },
  skill_navigator: {
    id: 'skill_navigator',
    ...
    fields: [
      { key: 'current_skills', label: 'Current Skills', placeholder: 'e.g., Python, SQL, basic ML, Power BI...', type: 'textarea', rows: 3 },
      { key: 'dream_role', label: 'Dream Role', placeholder: 'e.g., Azure AI Solutions Architect...', type: 'text' },
      { key: 'experience_level', label: 'Experience Level', placeholder: 'e.g., 2 years in data analytics, fresher in ML...', type: 'text' },
      { key: 'time_commitment', label: 'Weekly Time Available (Optional)', placeholder: 'e.g., 10 hours/week, weekends only...', type: 'text' },
    ],
  },
}

interface Field { key: string; label: string; placeholder: string; type: string; rows?: number }
...
export default function CareerCoachPage() {
  ...
  const [agentProgress, setAgentProgress] = useState<Record<string, { currentStep: number; stepStatus: string; stepData: Record<number, string> }>>({})
  ...
  const handleFieldChange = (key: string, value: string) => setFields(p => ({ ...p, [key]: value }))
  ...
  const hasInput = () => agent ? agent.fields.some(f => fields[f.key]?.trim()) : false
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  <div key={field.key}>
                    ...
                    {field.type === 'textarea' ? (
                      <textarea
                        ...
                        rows={field.rows || 3}
                        className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-dark-700 placeholder-dark-300 focus:border-primary-400 focus:ring-1 focus:ring-primary-400 outline-none resize-none"
                      ...
                      <input
                        ...
                        onChange={e => handleFieldChange(field.key, e.target.value)}
                        placeholder={field.placeholder}
                        className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-dark-700 placeholder-dark-300 focus:border-primary-400 focus:ring-1 focus:ring-primary-400 outline-none"
                        disabled={status === 'streaming'}
                      />
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between pt-4 mt-4 border-t border-gray-200">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    disabled={!hasInput() || status === 'streaming'}
            ...
            {Object.keys(results).length > 0 && (
              <div ref={resultsRef} className={`rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden ${expanded ? 'fixed inset-4 z-50' : ''}`}>
                ...
                {/* Markdown content */}
                <div className={`p-5 ${expanded ? 'max-h-[calc(100vh-140px)]' : 'max-h-[70vh]'} overflow-y-auto`}>
            ...
            {status === 'idle' && (
              <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <ul className="space-y-2 text-xs text-dark-500">
                  {['Fill in as many fields as possible for detailed analysis', 'Include specific skills, years of experience, and metrics', 'Be specific about your target role or career goal', 'Results include real-time market data and verified resource links'].map((t, i) => (
