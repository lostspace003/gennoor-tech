...
export default function CareerCoachPage() {
  ...
  const validateDownloadForm = () => {
    const errs: string[] = []
    if (!downloadContact.name.trim()) errs.push('name')
    if (!downloadContact.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(downloadContact.email)) errs.push('email')
    if (!downloadContact.whatsapp.trim()) errs.push('whatsapp')
    setDownloadErrors(errs)
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      // Save contact info + download event to Azure
      if (agent) {
        await fetch('/api/career-session', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            sessionId,
      ...
      const html = `<div id="pdf-content" style="font-family:'Segoe UI',Arial,sans-serif;color:#1e293b;padding:0;max-width:700px;">
        <div style="text-align:center;margin-bottom:28px;padding-bottom:16px;border-bottom:3px solid #0284c7;">
          <h1 style="color:#0284c7;font-size:24px;margin:0 0 4px 0;letter-spacing:-0.5px;">Gennoor Tech</h1>
          <p style="color:#64748b;font-size:11px;margin:0;">AI-Powered Career Coaching Report</p>
        </div>
        <div style="background:#f0f9ff;border-radius:8px;padding:14px 18px;margin-bottom:24px;">
          <table style="width:100%;font-size:12px;color:#334155;border-collapse:collapse;">
            <tr><td style="padding:3px 0;font-weight:600;width:100px;">Name</td><td>${downloadContact.name}</td></tr>
            <tr><td style="padding:3px 0;font-weight:600;">Email</td><td>${downloadContact.email}</td></tr>
            <tr><td style="padding:3px 0;font-weight:600;">WhatsApp</td><td>${downloadContact.whatsapp}</td></tr>
            <tr><td style="padding:3px 0;font-weight:600;">Agent</td><td>${agent?.name || 'Career Coach'}</td></tr>
            <tr><td style="padding:3px 0;font-weight:600;">Date</td><td>${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</td></tr>
          </table>
        </div>
        ${allResults}
        <div style="margin-top:32px;padding-top:12px;border-top:2px solid #e2e8f0;text-align:center;">
          <p style="color:#94a3b8;font-size:10px;margin:0;">Generated by Gennoor Tech AI Career Coach | gennoor.com</p>
          <p style="color:#94a3b8;font-size:10px;margin:2px 0 0;">This report is confidential and prepared exclusively for ${downloadContact.name}</p>
        </div>
      </div>
      <style>
        .md-content h1,.md-content h2,.md-content h3{color:#0c4a6e;margin:14px 0 6px;}
        .md-content h1{font-size:16px;} .md-content h2{font-size:14px;} .md-content h3{font-size:13px;}
        .md-content p{font-size:12px;line-height:1.6;margin:4px 0 8px;}
        .md-content ul,.md-content ol{font-size:12px;line-height:1.6;padding-left:20px;margin:4px 0 8px;}
        .md-content li{margin:2px 0;}
        .md-content strong{color:#0f172a;}
        .md-content code{background:#f1f5f9;padding:1px 4px;border-radius:3px;font-size:11px;}
        .md-content blockquote{border-left:3px solid #0284c7;padding-left:12px;margin:8px 0;color:#475569;font-style:italic;}
        .md-content table{width:100%;border-collapse:collapse;font-size:11px;margin:8px 0;}
        .md-content td,.md-content th{border:1px solid #e2e8f0;padding:4px 8px;text-align:left;}
        .md-content th{background:#f0f9ff;font-weight:600;}
      </style>`
      ...
      await html2pdf()
        .set({
          margin: [12, 12, 12, 12],
          filename: `Gennoor_Career_Report_${downloadContact.name.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.pdf`,
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {status === 'complete' && (
                        <button
                          ...
                          className="rounded-lg border border-primary-200 bg-primary-50 px-3 py-1.5 text-xs font-semibold text-primary-700 hover:bg-primary-100 transition-colors flex items-center gap-1.5"
                        >
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden animate-slideUp">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-primary-50">
              <div className="flex items-center gap-3">
                <div className="rounded-full bg-primary-100 p-2">
                  <svg className="h-5 w-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                </div>
                <div>
                  <h3 className="text-base font-bold text-primary-800">Download Your Report</h3>
                  <p className="text-xs text-primary-600">Fill in your details to get a branded PDF</p>
                </div>
              </div>
              <button
                onClick={() => !isGeneratingPdf && setShowDownloadForm(false)}
                className="rounded-full p-1.5 text-primary-500 hover:bg-primary-100 transition-colors"
              >
                <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={downloadContact.name}
                  onChange={e => { setDownloadContact(p => ({ ...p, name: e.target.value })); setDownloadErrors(p => p.filter(k => k !== 'name')) }}
                  placeholder="e.g., Jalal Ahmed"
                  className={`w-full rounded-lg border ${downloadErrors.includes('name') ? 'border-red-400 ring-1 ring-red-300 bg-red-50' : 'border-gray-300 bg-white'} px-3 py-2 text-sm text-dark-700 placeholder-dark-300 focus:border-primary-400 focus:ring-1 focus:ring-primary-400 outline-none`}
                  disabled={isGeneratingPdf}
                />
                {downloadErrors.includes('name') && <p className="text-xs text-red-500 mt-1">Name is required</p>}
              </div>
              ...
              <div>
                ...
                <input
                  type="email"
                  value={downloadContact.email}
                  onChange={e => { setDownloadContact(p => ({ ...p, email: e.target.value })); setDownloadErrors(p => p.filter(k => k !== 'email')) }}
                  placeholder="e.g., you@example.com"
                  className={`w-full rounded-lg border ${downloadErrors.includes('email') ? 'border-red-400 ring-1 ring-red-300 bg-red-50' : 'border-gray-300 bg-white'} px-3 py-2 text-sm text-dark-700 placeholder-dark-300 focus:border-primary-400 focus:ring-1 focus:ring-primary-400 outline-none`}
                  disabled={isGeneratingPdf}
                />
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
              ...
              >
                {isGeneratingPdf ? (
                  ...
                  <><svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg> Download PDF Report</>
                )}
              </button>
              <p className="text-[10px] text-dark-400 text-center mt-2">Your details are stored securely and never shared</p>
            </div>
          </div>
        </div>
      )}
