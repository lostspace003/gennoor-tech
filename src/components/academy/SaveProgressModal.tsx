...
export default function SaveProgressModal({ isVisible, onDismiss, onLoginSuccess, completedChapters }: SaveProgressModalProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 animate-slide-up">
      <div className="bg-[#0C1426] text-white px-4 sm:px-6 py-4 shadow-2xl">
        <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/60">Sign in to pick up where you left off on any device.</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setShowAuth(true)}
              className="px-5 py-2 bg-secondary-400 text-dark-900 text-sm font-bold rounded-lg hover:bg-secondary-300 transition-colors"
            >
              Sign In
            </button>
            <button
